...
export default function ManageBookings() {
  const { myTheaters } = useOutletContext();
  ...
  const fetchBookings = async () => {
    try {
      ...
      console.log('Fetching theater bookings...');
      // Fetch bookings for all managed theaters
      const response = await api.get('/bookings/my-theaters-bookings');
      console.log('Theater bookings response:', response);
      
      const bookingsData = response.data?.data?.bookings || [];
      const statsData = response.data?.data?.stats || {};
      
      console.log('Theater bookings data:', bookingsData);
      console.log('Theater stats:', statsData);
      
      setBookings(bookingsData);
      setStats({
        total: statsData.totalBookings || 0,
        paid: statsData.paidBookings || 0,
  ...
  if (!myTheaters || myTheaters.length === 0) {
    return (
      <div className="text-center py-16 bg-gray-900/30 border-2 border-dashed border-gray-700 rounded-lg">
        <FiHome className="mx-auto text-5xl text-gray-500 mb-3" />
        <h3 className="text-xl font-semibold text-white">No Theaters Assigned</h3>
        <p className="text-gray-400 mt-1">
          You don't have any theaters assigned yet. Contact admin to get theaters assigned.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-300 mt-1">
          View bookings for your managed theaters
        </p>
      </div>
      ...
      {filteredBookings.length === 0 ? (
        <div className="text-center py-16 bg-gray-900/30 border-2 border-dashed border-gray-700 rounded-lg">
          ...
          <p className="text-gray-400 mt-1">
            {filter === 'all' ? 'No bookings have been made yet for your theaters.' : `No ${filter} bookings found.`}
          </p>
        </div>
      ) : (
